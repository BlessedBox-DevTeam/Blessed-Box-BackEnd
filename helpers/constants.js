const MALE_GENDER_ID = 1;
const FEMALE_GENDER_ID = 2;
const UNLABELED_GENDER_ID = 3;
const TWO_TO_FOUR_YEARS_ID = 1;
const FIVE_TO_NINE_YEARS_ID = 2;
const TEN_TO_FOURTEEN_YEARS_ID = 3;

module.exports = {
  BETHLEHEM_RECOLLECTION_CENTER_ID: 1,
  // Genders
  MALE_GENDER_ID,
  FEMALE_GENDER_ID,
  UNLABELED_GENDER_ID,
  // Box Ages
  TWO_TO_FOUR_YEARS_ID,
  FIVE_TO_NINE_YEARS_ID,
  TEN_TO_FOURTEEN_YEARS_ID,
  // Transaction Status
  PENDING_STATUS_ID: 1,
  COMPLETED_STATUS_ID: 2,
  DECLINED_STATUS_ID: 3,
  // Role Codes
  SYSADMIN_ROLE_CODE: "SYSADMIN",
  MANAGER_ROLE_CODE: "MANAGER",
  // Roles Ids
  DONOR_ROLE_TYPE_ID: 2,
  // Permissions
  WRITE_QR_CODE_PERMISSION: "WRITE_QR_CODE",
  WRITE_RC_PERMISSION: "WRITE_RC",
  ASSIGN_ROLE_PERMISSION: "ASSIGN_ROLE",
  WRITE_TRANSACTION_PERMISSION: "WRITE_TRANSACTION",
  READ_TRANSACTION_PERMISSION: "READ_TRANSACTION_DETAILS",
  EDIT_TRANSACTION__PERMISSION: "EDIT_TRANSACTION_DETAILS",
  DELETE_TRANSACTION_PERMISSION: "DELETE_TRANSACTION",
  READ_OWN_TRANSACTION_PERMISSION: "READ_OWN_TRANSACTIONS",
  READ_RC_BOX_COUNT_PERMISSION: "READ_RC_BOX_COUNT",

  // Socket events
  SOCKET_EVENT_NEW_TRANSACTION: "transaction:new",
  SOCKET_EVENT_TRANSACTION_UPDATED: "transaction:updated",
  SOCKET_EVENT_NEW_BOX_COUNT: "boxCount:new",

  // Mapas
  AGE_MAP: {
    "2-4": TWO_TO_FOUR_YEARS_ID,
    "5-9": FIVE_TO_NINE_YEARS_ID,
    "10-14": TEN_TO_FOURTEEN_YEARS_ID,
    All: [TWO_TO_FOUR_YEARS_ID, FIVE_TO_NINE_YEARS_ID, TEN_TO_FOURTEEN_YEARS_ID]
  },
  GENDER_MAP: {
    Male: MALE_GENDER_ID,
    Female: FEMALE_GENDER_ID,
    Unlabeled: UNLABELED_GENDER_ID,
    boy: MALE_GENDER_ID,
    girl: FEMALE_GENDER_ID,
    unlabeled: UNLABELED_GENDER_ID,
    All: [MALE_GENDER_ID, FEMALE_GENDER_ID, UNLABELED_GENDER_ID]
  }
};
